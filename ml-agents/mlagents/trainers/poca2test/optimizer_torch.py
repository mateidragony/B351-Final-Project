...
class POCA2TestSettings(OnPolicyHyperparamSettings):
    beta: float = 5.0e-3
    epsilon: float = 0.2
    lambd: float = 0.95
    num_epoch: int = 3
...
class TorchPOCA2TestOptimizer(TorchOptimizer):
    class POCA2TestValueNetwork(torch.nn.Module, Critic):
        """
        The POCA2TestValueNetwork uses the MultiAgentNetworkBody to compute the value
        and POCA2Test baseline for a variable number of agents in a group that all
        ...
        def baseline(
            ...
            """
            The POCA2Test baseline marginalizes the action of the agent associated with self_obs.
    ...
    def __init__(self, policy: TorchPolicy, trainer_settings: TrainerSettings):
        ...
        :param policy: A TorchPolicy object that will be updated by this POCA2Test Optimizer.
        ...
        self._critic = TorchPOCA2TestOptimizer.POCA2TestValueNetwork(
            reward_signal_names,
        ...
        self.hyperparameters: POCA2TestSettings = cast(
            POCA2TestSettings, trainer_settings.hyperparameters
        )
    ...
    def create_reward_signals(
        ...
        for reward_signal in reward_signal_configs.keys():
            if reward_signal != RewardSignalType.EXTRINSIC:
                logger.warning(
                    f"Reward signal {reward_signal.value.capitalize()} is not supported with the POCA2Test trainer; "
                    "results may be unexpected."
                )
        super().create_reward_signals(reward_signal_configs)
        # Make sure we add the groupmate rewards in POCA2Test, so agents learn how to help each
